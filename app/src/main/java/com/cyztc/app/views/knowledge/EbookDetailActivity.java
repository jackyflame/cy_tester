...
public class EbookDetailActivity extends BaseActivity{
    ...
    private File eFile;
    private boolean isDoenload = false;

    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_ebook_detail_layout);
        ebookid = getIntent().getStringExtra("ebookid");
        setBackView();
        setTitle("电子书");
        showDataLoadMsg("数据加载中");
        getVideoDetail(CyApplication.getInstance().getUserBean().getStudentId(), ebookid);
    }
...
//        favoriBook(ebookid);
    ...
    public void onClickDownload(View view)
    {
        if(!isDoenload) {
            showLoadDialog("下载中");
            if (videoDetailBean != null && !TextUtils.isEmpty(videoDetailBean.getFile())) {
                getBaseFileUrl();
            } else {
        ...
        {
            try {
                Uri uri = Uri.parse("file://" + getFilePath(videoDetailBean.getFile()));
                Intent intent = new Intent();
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                intent.setAction(Intent.ACTION_VIEW);
                String type = getMIMEType(getFilePath(videoDetailBean.getFile()));
            ...
            {
                showToast("请先安装QQ阅读在打开电子书");
            }
        }
    }

    public static void startActivity(Context context, String ebookid)
    {
    ...
    public void getVideoDetail(String studentId, String resourceId)
    {
        BookApi.getInstance().getVideoDetail(studentId, resourceId, new HttpSubscriber<VideoDetailBean>(new SubscriberOnListener<VideoDetailBean>() {
            ...
            public void onSucceed(VideoDetailBean data) {
                ...
                tvStatus.setText("字数：" + data.getWordCount() + "字");
                eFile = new File(getFilePath(videoDetailBean.getFile()));
                if(eFile.exists())
                {
                    isDoenload = true;
    ...
    public void getBaseFileUrl()
    {
        ...
        x.http().get(requestParams, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                {
                    try {
                        result = java.net.URLEncoder.encode(videoDetailBean.getFile(), "UTF-8");
                        downloadFile(basefileurl.getData() + result, getFilePath(videoDetailBean.getFile()));
...
//        BookApi.getInstance().getBaseFileUrl(new HttpSubscriber<String>(new SubscriberOnListener<String>() {
//            @Override
...
//        }, this));
    ...
    private void downloadFile(final String url, String path) {
        MyLog.d("downloadPath:" + path);
        MyLog.d("downloadUrl:" + url);
        ...
        x.http().get(requestParams, new Callback.ProgressCallback<File>() {
            ...
            public void onLoading(long total, long current, boolean isDownloading) {
                String s = "下载进度:" + ((current * 100 / total)) + "%";
                showLoadDialog(s);
                MyLog.d(s);
            }
    ...
    public String getSavePath() {
        return Environment.getExternalStorageDirectory().getPath() + "/cyztc/ebooks/";
    }
    ...
    public String getFilePath(String url)
    {
        ...
        if(names.length > 0) {
            return getSavePath() + names[names.length - 1];
        }
        return "";
    }
