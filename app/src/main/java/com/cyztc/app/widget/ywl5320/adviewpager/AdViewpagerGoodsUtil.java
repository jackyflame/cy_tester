 ...
 */
public class AdViewpagerGoodsUtil {
     ...
     * @param urls
     */
    public AdViewpagerGoodsUtil(Context context, ViewPager viewPager, List<? extends BaseBean> urls) {
     ...
     * @param urls
     */
    public AdViewpagerGoodsUtil(Context context, ViewPager viewPager, LinearLayout dotlayout, int dotsize, int dotoffset, List<? extends BaseBean> urls) {
        this.context = context;
        this.viewPager = viewPager;
     ...
     * @param urls
     */
    public AdViewpagerGoodsUtil(Context context, ViewPager viewPager, LinearLayout dotlayout, int dotsize, int dotoffset, List<? extends BaseBean> urls, int dotColor) {
        this.context = context;
        this.viewPager = viewPager;
        this.dotlayout = dotlayout;
        this.dotsize = dotsize;
    ...
    private void setImg(int length, List<? extends BaseBean> urls)
    {
        if (urls.size() > 0) {
            ...
            for (int i = 0; i < length; i++) {
                if (i < length - 2) {
                    final int index = i;
                    final GoodsBannerBean bean = (GoodsBannerBean) urls.get(i);
                    Glide.with(context) .load(HttpMethod.IMG_URL + bean.getPicture()).into(mImageViews[i + 1]);
//                    ImageLoad.getInstance().displayImage(context, mImageViews[i + 1], bean.getImage(), R.drawable.task_game_changfangxing, R.drawable.task_game_changfangxing);
            ...
            Glide.with(context) .load(HttpMethod.IMG_URL + ((GoodsBannerBean)urls.get(urls.size() - 1)).getPicture()).into(mImageViews[0]);
            Glide.with(context) .load(HttpMethod.IMG_URL + ((GoodsBannerBean)urls.get(0)).getPicture()).into(mImageViews[length - 1]);
        }
    }
    ...
    {
        void onItemClick(GoodsBannerBean bannerBean);
    }
