...
public class NavitationLayout extends RelativeLayout{
    ...
    private void setTitles(Context context, String[] titles, final boolean smoothScroll, int splitcolor, int topoffset, int bottomoffset)
    {
        this.textViews = new TextView[titles.length];
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(0,LayoutParams.MATCH_PARENT);
        params.weight = 1;
        ...
        LinearLayout.LayoutParams params2 = new LinearLayout.LayoutParams(dip2px(context, 0.5f),LayoutParams.MATCH_PARENT);
        params2.setMargins(0, dip2px(context, topoffset), 0, dip2px(context, bottomoffset));
        // 循环，根据标题栏动态生成TextView来显示标题，每个标题栏的宽度比例为1:1,其中的内容居中。
        for(int i = 0; i < titles.length; i++)
        {
            final int index = i;
            ...
            if(i < titles.length - 1)
            {
                ...
                view.setBackgroundColor(context.getResources().getColor(splitcolor));
                titleLayout.addView(view, params2);
            }
        }
     ...
     * @param color
     */
    public void setBgLine(Context context, float height, int color)
    {
        int theight = dip2px(context,height);
        LayoutParams layoutParams = new LayoutParams(LayoutParams.MATCH_PARENT, theight);
        bgLine = new View(context);
        bgLine.setLayoutParams(layoutParams);
        bgLine.setBackgroundColor(context.getResources().getColor(color));

        LayoutParams lp = new LayoutParams(LayoutParams.MATCH_PARENT, theight);
    ...
    public void setViewPager(final Context context, String[] titles, ViewPager viewPager, final int unselectedcolor, final int setectedcolor, int txtUnselectedSize, final int txtSelectedSize, final int currentPosition, int widOffset, boolean smoothScroll, int splitcolor, int topoffset, int bottomoffset)
    {
        this.viewPager = viewPager;
        this.txtUnselectedColor = unselectedcolor;
        this.txtSelectedColor = setectedcolor;
        ...
        viewPager.setCurrentItem(currentPosition);
        setTitles(context, titles, smoothScroll, splitcolor, topoffset, bottomoffset);
        setUnselectedTxtColor(context, unselectedcolor, txtUnselectedSize);
        setSelectedTxtColor(context, setectedcolor, txtSelectedSize, currentPosition);
        viewPager.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            @Override
