...
public class AdViewpagerUtil {
     ...
     * @param urls
     */
    public AdViewpagerUtil(Context context, ViewPager viewPager, List<? extends BaseBean> urls) {
     ...
     * @param urls
     */
    public AdViewpagerUtil(Context context, ViewPager viewPager, LinearLayout dotlayout, int dotsize, int dotoffset, List<? extends BaseBean> urls) {
        this.context = context;
        this.viewPager = viewPager;
     ...
     * @param urls
     */
    public AdViewpagerUtil(Context context, ViewPager viewPager, LinearLayout dotlayout, int dotsize, int dotoffset, List<? extends BaseBean> urls, int dotColor) {
        this.context = context;
        this.viewPager = viewPager;
        this.dotlayout = dotlayout;
        this.dotsize = dotsize;
    ...
    public void updateAdImgs(List<? extends BaseBean> urls)
    {
        this.urls = urls;
        initVps();
    }
    ...
    private void setImg(int length, List<? extends BaseBean> urls)
    {
        if (urls.size() > 0) {
            ...
            for (int i = 0; i < length; i++) {
                if (i < length - 2) {
                    final int index = i;
                    final BannerBean bean = (BannerBean) urls.get(i);
                    Glide.with(context) .load(bean.getPicture()).into(mImageViews[i + 1]);
//                    ImageLoad.getInstance().displayImage(context, mImageViews[i + 1], bean.getImage(), R.drawable.task_game_changfangxing, R.drawable.task_game_changfangxing);
            ...
            Glide.with(context) .load(((BannerBean)urls.get(urls.size() - 1)).getPicture()).into(mImageViews[0]);
            Glide.with(context) .load(((BannerBean)urls.get(0)).getPicture()).into(mImageViews[length - 1]);
        }
    }
